...
def wait(func):
    ...
    def wrapper(*args, **kwargs):
        logFile.info("当前Page: {} 操作: {} 控件名: {}".format(
            args[1].file, func.__name__, "->".join([str(x) for x in args[1:]])))
        try:
            WebDriverWait(args[0], G.TIMEOUT).until(EC.element_to_be_clickable(
...
def found(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
        logFile.info("页面: {} 操作: {} 控件名: {}".format(
            args[1].file, func.__name__, "->".join([str(x) for x in args[1:]])))
        try:
            WebDriverWait(args[0], G.TIMEOUT).until(EC.visibility_of_any_elements_located(
                (getattr(By, args[1].method), args[1].value)))
        except ElementNotVisibleException:
            logFile.error("等待元素超时! \n文件名: {}\n函数名: {}\n控件名: {} 定位: [{} -> {}]".format(
                args[1].file, func.__name__, args[1].name, args[1].method, args[1].value))
...
def update_pic_info(case_id, filename):
    if hasattr(G, "screenshot"):
        getattr(G, "screenshot").update({case_id: generate_bs64(filename)})
    else:
        setattr(G, "screenshot", {case_id: generate_bs64(filename)})
...
def pic(*params):
    if not callable(params[0]):
        ...
        def decorator(func):
            @wraps(func)
            def wrapper(*args, **kwargs):
                logFile.info("当前运行测试用例: {} 函数: {}".format(args[0].__class__.__name__, func.__name__))
                ...
                for i in range(1, re_run + 1):
                    try:
                        ...
                        error = e
                        logFile.warning("{}_{}用例第{}次失败: {}".format(
        ...
        @wraps(params[0])
        def wrapper(*args, **kwargs):
            logFile.info("当前运行测试用例id: {} 名称: {}".format(args[0].__class__.__name__, params[0].__name__))
            driver = args[0].driver
            case_id = args[0].case_id
            error = None
            for i in range(1, 3):
...
def screenshot(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
        logFile.info("当前运行测试用例id: {} 名称: {}".format(args[0].__class__.__name__, func.__name__))
        driver = args[0].driver
        case_id = args[0].case_id
        try:
            return func(*args, **kwargs)
            ...
            error = e
            logFile.warning("{}_{}用例运行失败: {}".format(
